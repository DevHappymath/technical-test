...
| Thông số | Giá trị |
|---|---|
| Địa chỉ (Authority) | `https://auth-sandbox.netlify.app/` |
| Discovery document | `https://auth-sandbox.netlify.app/.well-known/openid-configuration` |
...
| `iss` | Nơi phát hành token | `https://auth-sandbox.netlify.app/` |
...
Email mời làm bài kèm file `tokens.txt` chứa **4 access token** đã phát sẵn (hạn sử dụng ghi ngay đầu file). Dán thẳng vào header — hoặc vào nút **Authorize** trên Swagger — là chạy được:
